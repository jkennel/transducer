...
library(viridis)
# params <- list()
# params$rbr_files <- '/media/kennel/Seagate Expansion Drive/rbr_g360'
# params$rds_file <- '/media/kennel/Seagate Expansion Drive/rbr_g360meta'
...
if(length(fn) > 0) {
  ...
  rbr_sum <- summarize_files(fn, 
                             output_folder = '/media/kennel/Seagate Expansion Drive/rbr_g360meta/')
  ...
  calibration <- rbr_sum[!is.na(type), (calibration[[1]]), 
                         by = list(file_name, serial, 
                                   start_date = as.Date(start), 
                                   end_date = as.Date(end), type)]
  
  # rbr_sum[, str(calibration[1]), list(file_name, type)]
...
coverage <- unique(rbr_table[, list(file_name, start_date, end_date)])
...
rbr_sum <- rbr_sum[!wh]
fn <- unique(rbr_sum$file_name)
# fn <- fn[1:10]
for (i in seq_along(fn)) {
  p <- list()
  
  rbr_sub <- rbr_sum[file_name == fn[i]]
  # calibration_sub <- calibration[file_name == fn[i]]
